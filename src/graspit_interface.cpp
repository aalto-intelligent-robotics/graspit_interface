...
namespace GraspitInterface
{
	...
	for(int i=0 ; i < mesh.vertices.size();i++)
	{
	    ...
	    //pcl_mesh.cloud.data.push_back(new
	    //pcl::PointXYZ(p.x,p.y,p.z));
	    pcl::PointXYZ *point = new
		pcl::PointXYZ(p.x,p.y,p.z);
	...
	add_objects_client =
	    nh->serviceClient<graspit_interface::ImportGraspableBody>("importGraspableBody");
	get_graspable_bodies_client =
	    nh->serviceClient<graspit_interface::GetGraspableBodies>("getGraspableBodies");
	save_world_client =
    ...
    void GraspitInterface::onObjectRecButtonButtonPressed()
    {
	ROS_INFO("Sending ObjectRec Goal");
	RunObjectRecognitionGoal goal;
	recognizeObjectsActionClient->sendGoal(goal, boost::bind(&GraspitInterface::objectRecognitionCB, this, _1, _2),
    ...
    void GraspitInterface::onCaptureSceneButtonPressed()
    {
	...
	else
	{
	    ROS_INFO("Recieved %lu mean meshes and %lu sampled meshes",
		    result->mean_meshes.size(),
		    result->sample_meshes.size());
	    ...
	    //    addMesh(new_mesh_index, result->meshes.at(i), offset);
	    //}
		...
		for (int j =0; j < result->sample_meshes.size(); j++){
		    addMesh(mesh_idx_1+QString::number(j), result->sample_meshes.at(j), offset, false);
		}
	    }
	}
    ...
    void GraspitInterface::onCompleteShapeButtonPressed()
    {
	ROS_INFO("onCompleteShapeButtonPressed\n");
	graspit_interface::CompleteMeshGoal goal;
	/*
	   int numSelectedBodies = graspitCore->world->getNumSelectedBodies();
	   if (numSelectedBodies == 0)
	   {
	   ROS_INFO("No Selected Bodies!\n");
	   return;
	   }
	   selected_body = graspitCore->world->getSelectedBody(0);
	 */
	...
	complete_mesh_client->sendGoal(goal,  boost::bind(&GraspitInterface::completeMeshCB, this, _1, _2),
		actionlib::SimpleActionClient<graspit_interface::CompleteMeshAction>::SimpleActiveCallback(),
		actionlib::SimpleActionClient<graspit_interface::CompleteMeshAction>::SimpleFeedbackCallback());
    }



    void GraspitInterface::completeMeshCB(const actionlib::SimpleClientGoalState& state,
	    const graspit_interface::CompleteMeshResultConstPtr& result)
    {
	...
	//graspitCore->world->deselectAll();
	//graspitCore->world->destroyElement(selected_body, true);

	//int new_mesh_index = graspitCore->world->getNumGB() + 1;
	//addMesh(new_mesh_index, result->completed_mesh, offset);
    }
    void GraspitInterface::objectRecognitionCB(const actionlib::SimpleClientGoalState& state,
	    const graspit_msgs::RunObjectRecognitionResultConstPtr& result)
    {
	std::for_each(result->object_info.begin(),
		result->object_info.end(),
		boost::bind(&GraspitInterface::addObject, this, _1));
    ...
    void GraspitInterface::addToWorld(const QString modelname, const QString object_name, const transf object_pose)
    {
    ...
    bool GraspitInterface::addObjectCB(graspit_interface::AddObject::Request &request,
    ...
    void GraspitInterface::addMesh(QString mesh_index, shape_msgs::Mesh mesh, geometry_msgs::Vector3
	    offset, const bool is_mean)
    {
	std::vector<int> triangles;
	std::vector<position> vertices;
	...
	QString fullfilepath = QString(getenv("GRASPIT")) + QString("/models/objects/");
	if (!is_mean){
		...
		saveMesh(fullfilepath, b->getName(), pcl_mesh);
	    }
	    else
	    {
    ...
    void GraspitInterface::saveMesh(const QString filepath, const QString filename,const  pcl::PolygonMesh* pcl_mesh){
